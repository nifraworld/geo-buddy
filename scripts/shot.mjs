...
try {
  ...
  for (const sc of screens) {
    ...
    if (name === "quiz") { await evalJs(ws, `import("/src/engine.js").then((m) => { const ty = "${sub || "wsh"}"; const src = ty.startsWith("lm-") ? m.__test.landmarkQuestions(ty, ty === "lm-div" ? "bd" : "world") : m.__test.countryQuestions(ty); const qs = src.filter((q) => q.answerId === "076" || q.itemId === "076" || q.lmId === "taj-mahal").slice(0, 1); m.__test.startSession({ title: "test", questions: qs.length ? qs : src.slice(0, 1), clock: false, daily: false }); return new Promise((r) => setTimeout(r, 900)); })`); }
    if (name === "explore" && sub === "lm") { await evalJs(ws, `new Promise((r) => { const b = document.querySelector('[data-action="explore-scope"][data-scope="lm"]'); if (b) b.click(); setTimeout(r, 1200); })`); }
    if (name === "landmark") { await evalJs(ws, `import("/src/engine.js").then((m) => { m.__test.go("detail", { kind: "l", id: "${sub || "eiffel"}" }); return new Promise((r) => setTimeout(r, 1500)); })`); }
    if (name === "parent") { await evalJs(ws, `import("/src/engine.js").then((m) => { m.__test.D.settings.pin = "1234"; m.__test.render("parent"); return new Promise((r) => setTimeout(r, 1200)); })`); await evalJs(ws, `window.scrollTo(0, ${sub || 0}); 1`); }
