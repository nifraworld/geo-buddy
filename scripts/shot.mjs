...
try {
  ...
  for (const sc of screens) {
    ...
    if (name === "quiz") { await evalJs(ws, `import("/src/engine.js").then((m) => { const qs = m.__test.countryQuestions("${sub || "wsh"}").filter((q) => q.answerId === "076" || q.itemId === "076").slice(0, 1); m.__test.startSession({ title: "test", questions: qs.length ? qs : m.__test.countryQuestions("${sub || "wsh"}").slice(0, 1), clock: false, daily: false }); return new Promise((r) => setTimeout(r, 900)); })`); }
